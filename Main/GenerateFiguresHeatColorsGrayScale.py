...
def grayWithoutWhite():
    #'binary' is the name of cmap in gray scale starting from white
    gray = cm.get_cmap('binary', 512)
    newcmp = ListedColormap(gray(np.linspace(0.1, 1, 256)))
    
    return newcmp

def setLegendColors(graph):
...
def printFigures(inputFolder, outputFolder):
    ...
    colorTo0_1 = {27:0, 31:20, 49:40, 53:60, 93:80, 97:100}
        ...
        for mode in paramsMode:
            ...
            for walkingDistance in paramsWalkingDistance:
                ...
                for count, clients in enumerate(numClientsStart):
                    ...
                    legendTrue = plt.scatter(vehiclesAccumlatedTrue[startPosTrue:endPosTrue], clientsAccumlatedTrue[startPosTrue:endPosTrue], s=pointSizeTrue, c=colorsAccumulatedTrue[startPosTrue:endPosTrue], marker=markerTrue, cmap=grayWithoutWhite())
                    ...
                    legendFalse = plt.scatter(vehiclesAccumlatedFalse[startPosFalse:endPosFalse], clientsAccumlatedFalse[startPosFalse:endPosFalse], s=pointSizeFalse, c=colorsAccumulatedFalse[startPosFalse:endPosFalse], marker=markerFalse, cmap=grayWithoutWhite())
                    #It is needed to set the min and max clim because are made more than one scatter                
                    plt.clim(min(ticksConverted), max(ticksConverted))
